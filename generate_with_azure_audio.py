...
narration_source = os.environ.get('NARRATION_SOURCE', 'slide_notes')  # 'slide_notes' or 'external_file'
narration_file = os.environ.get('NARRATION_FILE', 'narration_notes.py')
...
def load_narration_notes(narration_file):
    """Load narration notes from a separate Python file (legacy support)"""
    try:
        # Load the narration file dynamically
        spec = importlib.util.spec_from_file_location("narration_module", narration_file)
        narration_module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(narration_module)
        
        if hasattr(narration_module, 'narration_notes'):
            return narration_module.narration_notes
        else:
            print(f"Warning: No 'narration_notes' found in {narration_file}")
            return {}
    except Exception as e:
        print(f"Error loading narration notes from {narration_file}: {e}")
        return {}
...
# Load narration based on configuration
if narration_source.lower() == 'external_file':
    print(f"\nLoading narration from external file: {narration_file}")
    narration_notes = load_narration_notes(narration_file)
    if narration_notes:
        print(f"Loaded {len(narration_notes)} narration entries from external file")
    else:
        print("No narration found in external file, falling back to slide notes")
        narration_notes = extract_narration_from_slides(presentation)
else:
    # Default: extract from slide notes
    print(f"\nExtracting narration from PowerPoint slide notes...")
    narration_notes = extract_narration_from_slides(presentation)
    
    # If no narration found in slides, try loading from external file (fallback)
    if not narration_notes and os.path.exists(narration_file):
        print(f"No narration found in slide notes, trying external file: {narration_file}")
        narration_notes = load_narration_notes(narration_file)
        if narration_notes:
            print(f"Loaded {len(narration_notes)} narration entries from external file")

if not narration_notes:
    print("Warning: No narration found in slide notes or external file. Videos will be silent.")
else:
...
print(f"- Original slides: {slides_dir}/")
print(f"- Audio files: audio_clips/")
print("\nCleaned up:")
print("- slide_images/ (processed slides)")
print("- test_audio/ (test files)")
print(f"\n🎬 Your final video is ready: {output_video_name}")
